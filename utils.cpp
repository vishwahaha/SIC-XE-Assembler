...
string expand_string(string data, int length, char fill_char, bool back = false)
{
  if (back)
  {
...
string int_to_string_hex(int x, int fill = 5)
{
  stringstream s;
...
void read_first_non_white_space(string line, int &index, bool &status, string &data, bool read_till_end = false)
{
  data = "";
  status = true;
  if (read_till_end)
